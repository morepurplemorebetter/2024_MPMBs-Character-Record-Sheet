	...
	if (CUflat.indexOf("stats") !== -1 || CurrentUpdates.types.indexOf("testasi") !== -1) {
		...
		if (AbilityScores_Button(true)) { // sets tooltip for stats and returns true if anything changed
			...
			if (CUflat.indexOf("stats") !== -1) {
				var statChanges = [];
				if (CurrentUpdates.types.indexOf("statsbackground") !== -1) statChanges.push(toUni("Background"));
				if (CurrentUpdates.types.indexOf("statsrace") !== -1) statChanges.push(toUni("Species"));
...
function gatherWeaponVars(inputObjectKey, inputObject) {
	...
	var gatherVars = {
		...
		get isWeapon() {
			return this.theWea.isNotWeapon ? false : /simple|martial|exotic|firearm|improvised weapon/i.test(this.theWea.type) ? true : !this.isSpell;
		},
		get isMeleeWeapon() {
