...
function ApplyCompRace(newRace, prefix, sCompType) {
	...
	if (aCrea.typeFound === "race") {// do the following if a race was found
		...
		if (aCrea.resistances) { // 2024 notation
			strFeatures.push("##\u25C6 Resistances##. " + aCrea.resistances + ".");
		}
		...
		if (aCrea.immunities) { // 2024 notation
			strFeatures.push("##\u25C6 Immunities##. " + aCrea.immunities + ".");
		}
		if (aCrea.languages) {
		...
		if (strFeatures) {
			...
			strFeatures = strFeatures.replace(/\[THIS\]/g, strRaceEntry);
		...
		for (var a = 0; a < arrProps.length; a++) {
			...
			for (var f = 0; f < feaA.length; f++) {
				...
				if (prop.useSpellDescription && SpellsList[prop.useSpellDescription] && SpellsList[prop.useSpellDescription].descriptionFull) {
					...
					if (strSchool && oSpell.subSchool) strSchool += " [" + oSpell.subSchool + "]";
					if (!strSchool) strSchool = oSpell.level == 0 ? "" : "spell";
					var useLevel = oSpell.level + (oSpell.psionic ? 10 : 0);
					var strLevel = spellLevelList[useLevel] ? spellLevelList[useLevel].replace(/s\b| \(.+\)/g, "") : "";
					if (oSpell.psionic) {
						var strSpellType = oSpell.level == 0 ? strLevel : strSchool.capitalize() + " " + strLevel.toLowerCase();
					} else {
...
// Remove a Wild Shape selection (only the first instance unless `bRemoveAll == true`)
function RemoveWildshape(input, inCrea, bRemoveAll, bExactMatch) {
	var prefixA = What("Template.extras.WSfront").split(",").splice(1);
	var inputLC = input.toLowerCase();
	inCrea = inCrea && CreatureList[inCrea] ? inCrea : ParseCreature(inputLC);
	if (!inCrea) return;
	for (var p = 0; p < prefixA.length; p++) {
		...
		for (var i = 1; i <= 4; i++) {
			...
			if (aShp == inputLC || (!bExactMatch && inCrea == aCrea)) {
...
function ApplyWeapon(inputText, fldName, isReCalc, onlyProf, forceRedo) {
	...
	if (aWea) {
		...
		if (QI && CurrentEvals.atkAdd) {
			...
			var isWeapon = theWea && theWea.isNotWeapon ? false : theWea ? /simple|martial|exotic|firearm|improvised weapon/i.test(theWea.type) : !isSpell;
...
function processWeaponProfs(AddRemove, srcNm, itemArr) {
	if (!itemArr) return;
	var weaponTypes = ["simple", "martial", "other", "otherEval"];
	for (var i = 0; i < itemArr.length; i++) {
		if (itemArr[i] && weaponTypes[i]) {
			SetProf("weapon", AddRemove, weaponTypes[i], srcNm, i != 2 ? false : isArray(itemArr[i]) && itemArr[i].length ? itemArr[i] : itemArr[i] ? [itemArr[i]] : false
			);
		}
	}
};
			...
			if (ProfObj == "other") {
				...
				for (var key in iSet) {
					...
					for (var i = 0; i < aWea.length; i++) {
						...
						if (theW && theW.type) {
						// Do not add this weapon to the string if already proficient in some other way
							if ( /natural|spell|cantrip/i.test(theW.type) ||
							(simpleProf  &&  /^simple$/i.test(theW.type)) ||
							(martialProf && /^martial$/i.test(theW.type))
							) continue; // already proficient
						} else {
						// Do not add this string if it is referencing simple or martial weapons, but already proficient in those (e.g. "Light Martial Weapon" proficiency from Monks)
							if ((simpleProf  &&  /simple/.test(aWeaI)) ||
							(martialProf && /martial/.test(aWeaI))
							) continue;
						}
