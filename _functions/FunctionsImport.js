...
function DirectImport(consoleTrigger) {
	...
	if (closeAlert) {
		...
		// Update the progress bar and stop the calculations
			...
			if (ImportField("Unit System")) {
				var isMetric = What("Unit System") === "metric";
				if (typePF) Value("Display.Weighttxt.LbKg", isMetric ? "KG" : "LB");
				if (isMetric) ImportField("Decimal Separator");
			}
			ImportField("DateFormat_Remember");
			...
			// Switching between the 5e (2014) and 5.5e (2024) rules or importing from an older beta doesn't work for the ability score automation, because things by the same name suddenly work very differently. Thus we are going to let the automation handle it.
				...
				// If using 5.5e (2024) rules, add a race column to ensure compatibility with v12 and earlier
					ASaddColumn("Species Bonus", "race", 2);
				}
