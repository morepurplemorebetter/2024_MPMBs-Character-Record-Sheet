...
function ToggleTextSize(size, linespacingSize, forceReset, removeLocalRichText) {
	...
	if (!tDoc.info.AdvLogOnly) {
		...
		if (typePF) {
			LinesFld.push("Background_Organisation.Left", "Background_Organisation.Right");
		} else {
...
function FindClasses(NotAtStartup, isFieldVal) {
	...
	for (var aClass in classes.known) {
		...
		for (var prop in classObj.features) {
		...
		if (subClObj && subClObj.features) {
			for (var prop in subClObj.features) {
		...
		for (var key in RaceList) {
			...
			if (kObj.variants && kObj.variants.length) {
				for (var v = 0; v < kObj.variants.length; v++) {
					...
					if (!sObj) {
						displayError(false, 'The racial variant "' + sVar + '" for the "' + kObj.name + '" species is missing from the RaceSubList object. The variant will be ignored for now, but please contact its author to have this issue corrected.');
...
function ApplyRace(inputracetxt, novardialog, newLvlForce) {
	if (IsSetDropDowns) return; // when just changing the dropdowns, don't do anything
	...
	if (CurrentRace.known && (CurrentRace.known !== oldRace[0] || CurrentRace.variant !== oldRace[1])) {
		...
		UpdateLevelFeatures("race", newLvlForce);
...
function AddInvStartingItems(oInput, bEquipment, b1stPage) {
	if (bEquipment) {
		...
		if (oInput.gold) Value("Gold Pieces", stringToNumber(What("Gold Pieces"), ",") + oInput.gold);
	}
		...
		if (CurrentRace.features) {
			...
			for (var f = 0; f < feaA.length; f++) {
				...
				try {
					...
					displayError(error, 'The "' + propFea.name + '" feature from the "' + CurrentRace.name + '" species produced the error below. Please share this error message with its author so they can correct this issue.');
				}
			}
		}
...
function SetToManual_Button(bSkipDialog, oDialogResults) {
...
function SetTheAbilitySaveDCs() {
	...
	var processAbility = function (sType, sName, obj) {
		...
		if (obj.abilitySaveAlt) {
			...
			var as1 = Number(What(AbilityScores.abbreviations[sSave - 1]));
			var as2 = Number(What(AbilityScores.abbreviations[obj.abilitySaveAlt - 1]));
