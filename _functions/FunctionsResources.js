...
function resourceDecisionDialog(atOpening, atReset, forceDDupdate) {
	...
	onlySRD = onlySRD.length === 1 && /^SRD/.test(onlySRD[0]);
...
function resourceSelectionDialog(type) {
	...
	switch (type) {
	 case "class" :
		...
		break;
	 case "race" :
		var theName = "Player Species";
		theExtra = ["\nRaces with variants will have all variants listed. The 'basic' version of the species is only listed so that you can exclude all variants except the basic version. Excluding the 'basic' version while still including any variant will have no effect.", 3];
		inclInArr = "all";
		var CSatt = "racesExcl";
...
function parseSource(srcObj, bInclOnly) {
	...
	// only add the excluded if not set to only do the included
